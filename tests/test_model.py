...
def test_modular_invalid_modulus_limit() -> None:
    with pytest.raises(
        RecordFluxError, match=r"^<stdin>:55:3: model: error: modulus exceeds limit \(2\*\*62\)$"
    ):
...
def test_range_invalid_size_exceeds_limit() -> None:
    with pytest.raises(
        RecordFluxError,
        match=r'^<stdin>:50:3: model: error: size of "T" exceeds limit \(2\*\*62\)$',
    ):
...
def test_enumeration_invalid_size_exceeds_limit() -> None:
    with pytest.raises(
        RecordFluxError,
        match=r'^<stdin>:8:20: model: error: size of "T" exceeds limit \(2\*\*62\)$',
    ):
